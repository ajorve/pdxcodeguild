...
def menu():
    ...
    filename = choices[int(user_selection)]
    return filename
...
def ari_score(scrubbed_text):
    ...
    return ari_scale[score]['grade_level']

def run_program():
    filename = menu()
    file_contents = open_file(filename)
    cleaned = text_clean(file_contents)
    scored = ari_score(cleaned)
    print(scored)
